...
import com.tim.spring0200.http.BaseHttpSSLSocketFactory.TrustAnyHostnameVerifier;
...
public class HttpClient {
	...
	public int send2(String sendData, String encoding) throws Exception {
		try {
			HttpURLConnection httpURLConnection = createConnection(encoding);
			if(null == httpURLConnection){
				throw new Exception("创建联接失败");
			}
			System.out.println("请求报文:[" + sendData + "]");
	...
	private HttpURLConnection createConnectionGet(String encoding) throws ProtocolException {
		...
		httpURLConnection.setRequestProperty("apikey", "00a2039c04bd902e7b5756ad2c258655");
		if ("https".equalsIgnoreCase(url.getProtocol())) {
			HttpsURLConnection husn = (HttpsURLConnection) httpURLConnection;
			husn.setSSLSocketFactory(new BaseHttpSSLSocketFactory());
			husn.setHostnameVerifier(new TrustAnyHostnameVerifier());//解决由于服务器证书问题导致HTTPS无法访问的情况
